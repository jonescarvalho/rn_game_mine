...
const getNeighbors = (board, row, column) => {
  ...
  rows.forEach(r => {
    columns.forEach(c => {
      // const neighbor = board[r] && board[r][c];
      // if (neighbor && !(neighbor.row === row && neighbor.column === column)) neighbors.push(neighbor);
...
const openField = (board, row, column) => {
  ...
  if (!field.opened) {
    ...
    if (field.mined) {
      ...
      const neighbors = getNeighbors(board, row, column)//.filter(n => !n.mined);
      field.nearMines = neighbors.filter(n => n.mined).length;
      
      //console.log(openedFields);
      if (field.nearMines > 0) {
        console.log(field.nearMines);
      }
    }
  }
};
...
const lostGame = board =>
  fields(board).filter(field => field.opened && field.mined).length > 0;
